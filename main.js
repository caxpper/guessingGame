...
function GuessingGame(){
    ...
    this.pick_number = function(){
        var random_number = this.getRandomInt(1, 15);
    ...
    this.the_number = this.pick_number();
    ...
    this.createDoors = function(){
        ...
        for(var row = 0; row < 3; row++) {
            ...
            for (var col = 1; col < 6; col++) {
                rowElem.append(this.createDoor((row*5)+col));
            }
     ...
     */
    this.createDoor = function(index){
        ...
        doorBorder.append($("<div>",{
            ...
            css:
                {
                    background: 'url('+this.getMonster()+')',
                    backgroundRepeat: 'no-repeat',
                    backgroundPosition: 'center center'
                }
        }));
    ...
    this.openDoor = function(field) {
        ...
        if (y.hasClass("doorOpened")) {
            ...
            if (the_guess > self.the_number) {
                setTimeout(function () {
                    monster.show();
                    $("#response_div").text("She isn\'t here! \r\n Look on the left!");
                    //power led on the doors
                    self.powerLed(y.parent(), power);
                }, 500);
            } else if (the_guess < self.the_number) {
                setTimeout(function () {
                    monster.show();
                    $("#response_div").text("She isn\'t here! \r\n Look on the right!");
                    //power led on the doors
     ...
     * @param oldPower
     */
    this.powerLed = function(elem,oldPower) {
